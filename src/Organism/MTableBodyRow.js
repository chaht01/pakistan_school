...
export default class MTableBodyRow extends React.Component {
	...
	renderActions() {
		...
		return (
			<TableCell
				...
				style={{ width: 42 * actions.length, padding: '0px 5px', ...this.props.options.actionsCellStyle }}
			>
	...
	renderDetailPanelColumn() {
		const CustomIcon = ({ icon, style }) =>
			typeof icon === 'string' ? <Icon style={style}>{icon}</Icon> : React.createElement(icon, { style });
		...
		if (typeof this.props.detailPanel == 'function') {
			return (
				<TableCell
					...
					style={{ width: 42, textAlign: 'center', ...this.props.options.detailCellStyle }}
				>
					<IconButton
						style={{
	...
	render() {
		...
		this.props.columns.filter(columnDef => columnDef.tableData.groupOrder > -1).forEach(columnDef => {
